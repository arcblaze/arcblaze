...
public class TaskResourceTest {
    ...
    public void testGet() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            final Task response = resource.get(securityContext, config, daoFactory, timer, task.getId());
    ...
    /**
     * Test how the resource responds to retrieving all tasks when none are available.
     * 
     * @throws DatabaseException
     *             if there is a database problem
     */
    ...
    public void testAllNoneAvailable() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            final Set<Task> tasks = resource.all(securityContext, config, daoFactory, timer, true, true, null, null);
    ...
    public void testAll() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            Set<Task> tasks = resource.all(securityContext, config, daoFactory, timer, true, true, null, null);
            assertNotNull(tasks);
            ...
            tasks = resource.all(securityContext, config, daoFactory, timer, true, true, 5, 0);
            assertNotNull(tasks);
            assertEquals(3, tasks.size());
            assertTrue(tasks.contains(active));
            assertTrue(tasks.contains(inactive));
            ...
            tasks = resource.all(securityContext, config, daoFactory, timer, true, true, 1, 1);
            assertNotNull(tasks);
            assertEquals(1, tasks.size());
            assertTrue(tasks.contains(inactive));

            tasks = resource.all(securityContext, config, daoFactory, timer, true, false, null, null);
            assertNotNull(tasks);
            ...
            tasks = resource.all(securityContext, config, daoFactory, timer, false, true, null, null);
            assertNotNull(tasks);
            assertEquals(2, tasks.size());
            assertTrue(tasks.contains(active));
            assertTrue(tasks.contains(inactive));
            ...
            tasks = resource.all(securityContext, config, daoFactory, timer, false, false, null, null);
            assertNotNull(tasks);
            assertEquals(1, tasks.size());
            assertTrue(tasks.contains(active));
            assertFalse(tasks.contains(inactive));
    ...
    public void testAdd() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            final AddResponse response = resource.add(securityContext, config, daoFactory, timer, task);
            ...
            assertEquals(1, daoFactory.getTaskDao().getAll(company.getId(), true, true, null, null).size());
        }
    }
    ...
    public void testActivate() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            resource.activate(securityContext, config, daoFactory, timer, new IdSet(task.getId()));

            assertTrue(daoFactory.getTaskDao().get(company.getId(), task.getId()).isActive());
        }
    }
    ...
    public void testDectivate() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            resource.deactivate(securityContext, config, daoFactory, timer, new IdSet(task.getId()));

            assertFalse(daoFactory.getTaskDao().get(company.getId(), task.getId()).isActive());
        }
    }
    ...
    public void testUpdate() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            final UpdateResponse response = resource.update(securityContext, config, daoFactory, timer, task);
    ...
    public void testDelete() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TaskResource resource = new TaskResource();
            resource.delete(securityContext, config, daoFactory, timer, new IdSet(task.getId()));

            assertEquals(0, daoFactory.getTaskDao().getAll(company.getId(), true, true, null, null).size());
        }
    }
}
