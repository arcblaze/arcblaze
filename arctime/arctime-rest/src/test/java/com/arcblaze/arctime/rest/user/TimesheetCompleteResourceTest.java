...
public class TimesheetCompleteResourceTest {
    ...
    public void testInvalidData() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TimesheetCompleteResource resource = new TimesheetCompleteResource();
            resource.complete(security, config, daoFactory, timer, timesheet.getId(), "data");
        }
    }
     ...
     */
    @Test
    public void testEmptyData() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final ArcTimeDaoFactory daoFactory = testDatabase.getDaoFactory();
            ...
            final TimesheetCompleteResource resource = new TimesheetCompleteResource();
            final CompleteResponse response = resource.complete(security, config, daoFactory, timer, timesheet.getId(),
                    "");
            ...
            assertTrue(next.getPayPeriod().contains(DateUtils.addDays(payPeriod.getEnd(), 2)));
            ...
            final Timesheet ts = dao.get(company.getId(), timesheet.getId(), AUDIT_LOGS, BILLS, HOLIDAYS, PAY_PERIODS,
                    TASKS, USERS);
            assertNotNull(ts);
     ...
     */
    @Test
    public void testEmptyDataWithPopulatedTimesheet() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final ArcTimeDaoFactory daoFactory = testDatabase.getDaoFactory();
     ...
     */
    @Test
    public void testCompleteDataIntoTimesheet() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final ArcTimeDaoFactory daoFactory = testDatabase.getDaoFactory();
            ...
            final String timesheetData = Bill.toTimesheetData(Arrays.asList(b1, b2, b3, b4, b5, b6, b7));
