...
public class HolidayResourceTest {
    protected Set<Holiday> getFederalHolidays(final Integer companyId) throws HolidayConfigurationException {
        ...
        for (final Map.Entry<String, String> entry : map.entrySet())
            holidays.add(new Holiday().setCompanyId(companyId).setDescription(entry.getKey())
    ...
    public void testGetInvalidId() throws DatabaseException, HolidayConfigurationException {
     ...
     */
    @Test
    public void testGet() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final ArcTimeDaoFactory daoFactory = testDatabase.getDaoFactory();
            ...
            final HolidayResource resource = new HolidayResource();
            final Holiday response = resource.get(securityContext, config, daoFactory, timer, holiday.getId());
    ...
    /**
     * Test how the resource responds to retrieving all holidays when none are available.
     * 
     * @throws DatabaseException
     *             if there is a database problem
     * @throws HolidayConfigurationException
     ...
     */
    @Test
    public void testAllNoneAvailable() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
            ...
            final HolidayResource resource = new HolidayResource();
            final Set<Holiday> holidays = resource.all(securityContext, config, daoFactory, timer);
     ...
     */
    @Test
    public void testAll() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
     ...
     */
    @Test
    public void testValidate() throws DatabaseException, HolidayConfigurationException {
        final MetricRegistry metricRegistry = new MetricRegistry();
        final Timer timer = metricRegistry.timer("test");
     ...
     */
    @Test
    public void testAdd() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
            ...
            final Holiday holiday = new Holiday().setCompanyId(company.getId()).setConfig("Jan 1st")
                    .setDescription("New Years");

            final HolidayResource resource = new HolidayResource();
            final AddResponse response = resource.add(securityContext, config, daoFactory, timer, holiday);
            ...
            assertEquals(1, daoFactory.getHolidayDao().getAll(company.getId()).size());
        }
    }
     ...
     */
    @Test(expected = BadRequestException.class)
    public void testUpdateNoId() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
     ...
     */
    @Test
    public void testUpdate() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
            ...
            final HolidayResource resource = new HolidayResource();
            final UpdateResponse response = resource.update(securityContext, config, daoFactory, timer, holiday);
     ...
     */
    @Test
    public void testDelete() throws DatabaseException, HolidayConfigurationException {
        final Config config = new Config();
        try (final TestDatabase testDatabase = new TestDatabase()) {
            testDatabase.load("hsqldb/arctime-db.sql");
            final User user = new User().setId(1).setCompanyId(1).setLogin("user");
            ...
            final HolidayResource resource = new HolidayResource();
            resource.delete(securityContext, config, daoFactory, timer, new IdSet(holiday.getId()));

            assertEquals(0, daoFactory.getHolidayDao().getAll(company.getId()).size());
        }
    }
}
