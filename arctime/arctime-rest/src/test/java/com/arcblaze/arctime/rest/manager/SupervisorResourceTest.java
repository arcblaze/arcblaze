...
public class SupervisorResourceTest {
    ...
    public void testGet() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            daoFactory.getSupervisorDao().add(company.getId(), user.getId(), true, spr1.getId());
            daoFactory.getSupervisorDao().add(company.getId(), user.getId(), false, spr2.getId());

            final SecurityContext securityContext = Mockito.mock(SecurityContext.class);
            Mockito.when(securityContext.getUserPrincipal()).thenReturn(user);

            final SupervisorResource resource = new SupervisorResource();
            final Set<Supervisor> supervisors = resource.get(securityContext, config, daoFactory, timer, user.getId());
    ...
    public void testAddNonExistent() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final SupervisorResource resource = new SupervisorResource();
            final AddResponse response = resource.add(securityContext, config, daoFactory, timer, user.getId(), 1234,
                    true);

            assertNotNull(response);
            assertFalse(response.success);

            final Set<Supervisor> supervisors = daoFactory.getSupervisorDao().getSupervisors(company.getId(),
                    user.getId());
    ...
    public void testAdd() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final SupervisorResource resource = new SupervisorResource();
            final AddResponse response = resource.add(securityContext, config, daoFactory, timer, user.getId(),
                    spr.getId(), true);
    ...
    public void testDeleteNonExistent() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final SupervisorResource resource = new SupervisorResource();
            final DeleteResponse response = resource.delete(securityContext, config, daoFactory, timer, user.getId(),
    ...
    public void testDelete() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            daoFactory.getUserDao().add(user, spr);
            daoFactory.getSupervisorDao().add(company.getId(), user.getId(), true, spr.getId());
