...
import com.arcblaze.arctime.rest.user.TimesheetNextResource.NextResponse;
...
public class TimesheetNextResourceTest {
    ...
    /**
     * Test how the resource responds with an invalid date.
     * 
    ...
    public void testNoTimesheetsAvailable() throws DatabaseException, ParseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final TimesheetNextResource resource = new TimesheetNextResource();
            final NextResponse response = resource.next(security, config, daoFactory, timer, "20140101");
            ...
            assertEquals("20140108", DateFormatUtils.format(ts.getPayPeriod().getBegin(), "yyyyMMdd"));
            assertEquals("20140114", DateFormatUtils.format(ts.getPayPeriod().getEnd(), "yyyyMMdd"));
    ...
    public void testTimesheetWithHolidays() throws DatabaseException, ParseException, HolidayConfigurationException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final Holiday holiday = new Holiday().setCompanyId(company.getId()).setDescription("holiday")
                    .setConfig(DateFormatUtils.format(DateUtils.addDays(payPeriod.getEnd(), 1), "MMM d"));
    ...
    public void testTimesheetWithUsers() throws DatabaseException, ParseException, HolidayConfigurationException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final PayPeriod payPeriod = new PayPeriod().setCompanyId(company.getId()).setType(PayPeriodType.WEEKLY)
                    .setBegin(DateUtils.parseDate("20140101", FMT)).setEnd(DateUtils.parseDate("20140107", FMT));
            daoFactory.getPayPeriodDao().add(payPeriod, payPeriod.getPrevious());
            ...
            final TimesheetNextResource resource = new TimesheetNextResource();
            final NextResponse response = resource.next(security, config, daoFactory, timer,
                    DateFormatUtils.format(payPeriod.getPrevious().getBegin(), "yyyyMMdd"));
            ...
            assertEquals(user.getId(), ts.getUserId());
            assertNotNull(ts.getPayPeriod());
            assertEquals("20140101", DateFormatUtils.format(ts.getPayPeriod().getBegin(), "yyyyMMdd"));
            assertEquals("20140107", DateFormatUtils.format(ts.getPayPeriod().getEnd(), "yyyyMMdd"));
            assertNotNull(ts.getBegin());
            assertEquals(ts.getBegin(), ts.getPayPeriod().getBegin());
            assertEquals("20140101", DateFormatUtils.format(ts.getPayPeriod().getBegin(), "yyyyMMdd"));
