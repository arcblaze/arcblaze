...
public class HolidayResource extends BaseResource {
    ...
    static class DeleteResponse {
        ...
        @XmlElement
        public String msg = "The specified holidays have been deleted " + "successfully.";
    }
     ...
     * 
     * @return the requested holiday (if in the same company as the current user)
     * 
    ...
    public Holiday get(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer,
            @PathParam("holidayId") final Integer holidayId) throws HolidayConfigurationException {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            if (holidayId == null)
                throw badRequest("Invalid null holiday id");

            return daoFactory.getHolidayDao().get(currentUser.getCompanyId(), holidayId);
     ...
     *            tracks performance metrics of this REST end-point
     * 
     * @return all of the available holidays in the same company as the current user
     * 
     * @throws HolidayConfigurationException
     *             if there is a problem parsing the holiday configuration information
     */
    @GET
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Set<Holiday> all(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer)
            throws HolidayConfigurationException {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            return daoFactory.getHolidayDao().getAll(currentUser.getCompanyId());
        } catch (final DatabaseException dbException) {
            throw dbError(config, currentUser, dbException);
        }
    }
    ...
    @Path("/common")
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Set<Holiday> common(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer)
            throws HolidayConfigurationException {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
    ...
    @Path("/validate")
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public ValidateResponse validate(@Context final Timer timer, @QueryParam("config") final String config) {
        try (final Timer.Context timerContext = timer.time()) {
            final ValidateResponse response = new ValidateResponse();
            response.success = true;
            try {
                response.day = HolidayCalculator.getDay(config,
                ...
                response.msg = "Invalid holiday configuration: " + badConfig.getMessage();
            }
            return response;
        }
    }
    ...
    public AddResponse add(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer, @Context final Holiday holiday) {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
    ...
    public DeleteResponse delete(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer,
            @HeaderParam("ids") final IdSet holidayIds) throws HolidayConfigurationException {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            if (holidayIds == null || holidayIds.isEmpty())
                throw badRequest("No holiday ids provided");
            final int deleted = daoFactory.getHolidayDao().delete(currentUser.getCompanyId(), holidayIds);
