...
public class UserResource extends BaseResource {
    ...
    static class DeleteResponse {
        ...
        @XmlElement
        public String msg = "The specified users have been deleted " + "successfully.";
    }
    ...
    public User get(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            return daoFactory.getUserDao().get(currentUser.getCompanyId(), userId);
     ...
     *            whether inactive user accounts should be included in the response
     * @param limit
     *            the maximum number of items to be retrieved
     * @param offset
     *            the offset into the items to be retrieved
     * 
     * @return all of the available users in the same company as the current user
     */
    @GET
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public AllResponse all(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            response.users = daoFactory.getUserDao().getAll(currentUser.getCompanyId(), includeInactive, limit, offset);
            daoFactory.getRoleDao().populateUsers(response.users);
            response.total = daoFactory.getUserDao().count(filter, includeInactive);
    ...
    public AddResponse add(@Context final SecurityContext security, @Context final Config config,
            @Context final DaoFactory daoFactory, @Context final Timer timer, @Context final User user) {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            if (user == null)
    ...
    public ActivateResponse activate(@Context final SecurityContext security, @Context final Config config,
            @Context final DaoFactory daoFactory, @Context final Timer timer, @HeaderParam("ids") final IdSet userIds) {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            if (userIds == null || userIds.isEmpty())
                throw badRequest("No user ids provided");
            final int updated = daoFactory.getUserDao().activate(currentUser.getCompanyId(), userIds);
    ...
    public DeactivateResponse deactivate(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            if (userIds == null || userIds.isEmpty())
                throw badRequest("No user ids provided");
            final int updated = daoFactory.getUserDao().deactivate(currentUser.getCompanyId(), userIds);
    ...
    public UpdateResponse update(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            if (StringUtils.isNotBlank(user.getHashedPass()))
                updated += daoFactory.getUserDao().setPassword(user.getId(), user.getHashedPass(), user.getSalt());

            final Set<Role> rolesToAdd = new TreeSet<>(user.getRoles());
            rolesToAdd.retainAll(currentUser.getRoles());
            user.setRoles(rolesToAdd);
            updated += daoFactory.getRoleDao().delete(user.getId(), currentUser.getRoles());
            updated += daoFactory.getRoleDao().add(user.getId(), user.getRoles());
    ...
    public DeleteResponse delete(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            if (userIds == null || userIds.isEmpty())
                throw badRequest("No user ids provided");
            final int deleted = daoFactory.getUserDao().delete(currentUser.getCompanyId(), userIds);
