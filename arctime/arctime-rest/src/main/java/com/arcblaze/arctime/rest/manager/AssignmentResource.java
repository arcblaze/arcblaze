...
public class AssignmentResource extends BaseResource {
	...
	public AddResponse add(@Context final SecurityContext security,
			@Context final Config config,
			@Context final ArcTimeDaoFactory daoFactory,
			@Context final Timer timer, @Context final Assignment assignment) {
		final User currentUser = (User) security.getUserPrincipal();
		try (final Timer.Context timerContext = timer.time()) {
	...
	public UpdateResponse update(@Context final SecurityContext security,
		...
		try (final Timer.Context timerContext = timer.time()) {
			...
			if (assignment.getUserId() == null)
				throw badRequest("A user id must be provided.");
			assignment.setCompanyId(currentUser.getCompanyId());
			final int updated = daoFactory.getAssignmentDao()
					.update(assignment);
	...
	public DeleteResponse delete(@Context final SecurityContext security,
			@Context final Config config,
			@Context final ArcTimeDaoFactory daoFactory,
			@Context final Timer timer, @HeaderParam("ids") final IdSet ids) {
		final User currentUser = (User) security.getUserPrincipal();
		try (final Timer.Context timerContext = timer.time()) {
			if (ids == null || ids.isEmpty())
				throw badRequest("An assignment id must be provided.");
			final int deleted = daoFactory.getAssignmentDao().delete(
					currentUser.getCompanyId(), ids);
