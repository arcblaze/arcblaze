...
/**
 * Provides access to {@link Task} objects within the REST resource classes,
...
public class AssignmentFactory extends BaseFactory<Assignment> {
	 ...
	 * @param itemName
	 *            the item name of the assignment, as used in the accounting
	 *            system
	 */
	public AssignmentFactory(@FormParam("id") final Integer id,
			@FormParam("companyId") final Integer companyId,
			@FormParam("taskId") final Integer taskId,
			@FormParam("userId") final Integer userId,
			@FormParam("begin") final String beginstr,
			@FormParam("end") final String endstr,
			@FormParam("laborCat") final String laborCat,
			@FormParam("itemName") final String itemName) {
		this.id = id;
	...
	public Assignment provide() {
		...
		if (StringUtils.isNotBlank(this.beginstr)) {
			try {
				...
				throw new IllegalArgumentException(
						"Invalid assignment begin date: " + this.beginstr,
						badDate);
			}
		}
		if (StringUtils.isNotBlank(this.endstr)) {
			try {
				...
				throw new IllegalArgumentException(
						"Invalid assignment end date: " + this.endstr, badDate);
			}
		}
		return assignment;
	}
