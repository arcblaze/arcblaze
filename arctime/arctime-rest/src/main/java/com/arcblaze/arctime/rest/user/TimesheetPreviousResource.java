...
public class TimesheetPreviousResource extends BaseResource {
	...
	public TimesheetResponse previous(@Context final SecurityContext security,
		...
		try (final Timer.Context timerContext = timer.time()) {
			...
			final PayPeriod payPeriod = ppdao.get(currentUser.getCompanyId(),
					begin);
			if (payPeriod == null)
			...
			Timesheet timesheet = dao.getForUser(currentUser.getId(), previous,
			...
			if (timesheet == null) {
				...
				timesheet = dao.getForUser(currentUser.getId(), previous,
						timesheetEnrichments);
			}
