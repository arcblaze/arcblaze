...
public class TimesheetCompleteResource extends BaseResource {
    private final static Logger log = LoggerFactory.getLogger(TimesheetCompleteResource.class);
    ...
    static class CompleteResponse {
        ...
        @XmlElement
        public final String msg = "The timesheet was completed successfully. " + "Moved to the next pay period.";
    ...
    public CompleteResponse complete(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            final Set<Enrichment> timesheetEnrichments = new LinkedHashSet<>(Arrays.asList(USERS, PAY_PERIODS, TASKS,
                    BILLS));
            ...
            if (!ppdao.exists(timesheet.getCompanyId(), nextPayPeriod.getBegin())) {
                ppdao.add(nextPayPeriod);
            }

            log.debug("Finding next timesheet");
            Timesheet next = dao.getForUser(currentUser.getId(), nextPayPeriod, timesheetEnrichments);
            if (next == null) {
                ...
                next = dao.get(currentUser.getCompanyId(), next.getId(), timesheetEnrichments);
            }
