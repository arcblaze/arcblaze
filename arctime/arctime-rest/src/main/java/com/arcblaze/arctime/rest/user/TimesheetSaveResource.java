...
public class TimesheetSaveResource extends BaseResource {
    private final static Logger log = LoggerFactory.getLogger(TimesheetSaveResource.class);
     ...
     * @param id
     *            the unique id of the timesheet into which this data will be saved
     * @param data
    ...
    public SaveResponse save(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            final Set<Enrichment> timesheetEnrichments = new LinkedHashSet<>(Arrays.asList(PAY_PERIODS, TASKS, BILLS));

            log.debug("Retrieving current timesheet");
            final TimesheetDao dao = daoFactory.getTimesheetDao();
            final Timesheet timesheet = dao.get(currentUser.getCompanyId(), id, timesheetEnrichments);
    ...
    protected static void saveTimesheet(final ArcTimeDaoFactory daoFactory, final Timesheet timesheet, final String data)
        ...
        for (final Bill bill : bills) {
            ...
            final Assignment assignment = bill.hasAssignmentId() ? task.getAssignment(bill.getAssignmentId()) : null;
            final Bill existing = assignment == null ? task.getBill(bill.getDay()) : assignment.getBill(bill.getDay());

            if (existing != null && !existing.getHours().equals(bill.getHours())) {
                bill.setId(existing.getId());
                bill.setUserId(existing.getUserId());
                billDao.update(bill);

                auditLogDao.add(getUpdatedLog(timesheet, task, assignment, existing, bill));
        ...
        for (final Task task : timesheet.getTasks()) {
            ...
            for (final Assignment assignment : task.getAssignments()) {
                for (final Bill bill : assignment.getBills()) {
                    ...
                    toDelete.add(bill.getId());
                    auditLogDao.add(getDeletedLog(timesheet, task, assignment, bill));
                }
            }
        }
    ...
    /**
     * A utility method for generating the audit log associated with changing hours associated with a bill.
     * 
     * @param timesheet
     ...
     */
    protected static AuditLog getUpdatedLog(final Timesheet timesheet, final Task task, final Assignment assignment,
            final Bill existing, final Bill updated) {
        ...
        return new AuditLog().setCompanyId(timesheet.getCompanyId()).setLog(log.toString())
                .setCompanyId(task.getCompanyId()).setTimesheetId(timesheet.getId());
    }

    /**
     * A utility method for generating the audit log associated with adding hours to a timesheet.
     * 
     * @param timesheet
     *            the timesheet being modified
     * @param task
     ...
     * @return an appropriate log message describing the changes
     */
    protected static AuditLog getAddedLog(final Timesheet timesheet, final Task task, final Assignment assignment,
            final Bill bill) {
        final StringBuilder log = new StringBuilder();
        log.append("Added ");
    ...
    /**
     * A utility method for generating the audit log associated with deleting hours associated with a bill.
     * 
     * @param timesheet
     *            the timesheet being modified
     * @param task
     ...
     * @return an appropriate log message describing the changes
     */
    protected static AuditLog getDeletedLog(final Timesheet timesheet, final Task task, final Assignment assignment,
            final Bill existing) {
        final StringBuilder log = new StringBuilder();
        log.append("Hours for task ");
        log.append(task.getDescription());
