...
public class SupervisorResource extends BaseResource {
    ...
    static class DeleteResponse {
        ...
        @XmlElement
        public String msg = "The specified supervisors have been deleted " + "successfully.";
    }
     ...
     * @param userId
     *            the unique id of the user for which supervisors will be retrieved
     * 
     * @return the requested supervisors (if in the same company as the current user)
     */
    @GET
    @Path("{userId:\\d+}")
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Set<Supervisor> get(@Context final SecurityContext security, @Context final Config config,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            return daoFactory.getSupervisorDao().getSupervisors(currentUser.getCompanyId(), userId);
    ...
    public AddResponse add(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer,
            @FormParam("userId") final Integer userId, @FormParam("supervisorId") final Integer supervisorId,
        ...
        try (final Timer.Context timerContext = timer.time()) {
            ...
            final int added = daoFactory.getSupervisorDao().add(currentUser.getCompanyId(), userId, primary,
                    supervisorId);
     ...
     *            tracks performance metrics of this REST end-point
     * @param userId
     *            the unique id of the user for which supervisors will be deleted
     * @param supervisorIds
    ...
    public DeleteResponse delete(@Context final SecurityContext security, @Context final Config config,
            @Context final ArcTimeDaoFactory daoFactory, @Context final Timer timer,
            @HeaderParam("userId") final Integer userId, @HeaderParam("supervisorIds") final Set<Integer> supervisorIds) {
        final User currentUser = (User) security.getUserPrincipal();
        try (final Timer.Context timerContext = timer.time()) {
            if (userId == null)
                throw badRequest("A user id must be provided.");
            ...
            final int deleted = daoFactory.getSupervisorDao().delete(currentUser.getCompanyId(), userId, supervisorIds);
