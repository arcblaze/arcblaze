...
/**
 * Provides access to {@link User} objects within the REST resource classes, based on parameters from the web request.
 */
public class UserFactory extends BaseFactory<User> {
     ...
     */
    public UserFactory(@FormParam("id") final Integer id, @FormParam("companyId") final Integer companyId,
            @FormParam("login") final String login, @FormParam("password") final String password,
            @FormParam("confirm") final String confirm, @FormParam("email") final String email,
            @FormParam("firstName") final String firstName, @FormParam("lastName") final String lastName,
            @FormParam("active") final Boolean active, @FormParam("roles") final Set<String> roles) {
        this.id = id;
