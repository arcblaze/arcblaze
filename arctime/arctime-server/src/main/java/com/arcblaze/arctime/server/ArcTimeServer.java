...
public class ArcTimeServer extends BaseServer {
     ...
     *             if there is a problem loading the system configuration information
     */
     ...
     */
    @Override
    public Class<? extends BaseApplication> getApplicationClass(final Config config) {
    ...
    public Set<Role> getSystemRoles(final Config config) {
        return new TreeSet<>(Arrays.asList(new Role("USER"), new Role("MANAGER"), new Role("SUPERVISOR"), new Role(
                "PAYROLL"), new Role("FINANCE"), new Role("ADMIN")));
    }
     ...
     */
    @Override
    public Collection<BackgroundTask> getBackgroundTasks(final Config config, final MetricRegistry metricRegistry,
