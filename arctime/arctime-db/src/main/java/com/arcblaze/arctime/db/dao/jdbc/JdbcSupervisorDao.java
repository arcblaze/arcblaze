...
public class JdbcSupervisorDao extends JdbcUserDao implements SupervisorDao {
    ...
    @Override
    protected Supervisor fromResultSet(final ResultSet rs, final boolean includePass) throws SQLException {
        final Supervisor supervisor = new Supervisor(super.fromResultSet(rs, includePass));
    ...
    public int add(final Integer companyId, final Integer userId, final boolean primary, final Integer... supervisorIds)
        ...
        return this.add(companyId, userId, primary, supervisorIds == null ? null : Arrays.asList(supervisorIds));
    }
    ...
    public int add(final Integer companyId, final Integer userId, final boolean primary,
        ...
        final String sql = "INSERT INTO supervisors (company_id, user_id, " + "supervisor_id, is_primary) "
                + "SELECT u.company_id, u.id, s.id, ? FROM users u " + "JOIN users s ON (u.company_id = s.company_id) "
    ...
    public int delete(final Integer companyId, final Integer userId, final Integer... supervisorIds)
            throws DatabaseException {
        return this.delete(companyId, userId, supervisorIds == null ? null : Arrays.asList(supervisorIds));
    }
    ...
    public int delete(final Integer companyId, final Integer userId, final Collection<Integer> supervisorIds)
        ...
        final String sql = "DELETE FROM supervisors WHERE company_id = ? AND " + "user_id = ? AND supervisor_id = ?";
