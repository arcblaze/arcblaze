...
public class ArcTimeDaoFactory extends DaoFactory {
    ...
    /**
     * @return an {@link AssignmentDao} based on the currently configured database
     */
    public AssignmentDao getAssignmentDao() {
        if (this.cachedAssignmentDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedAssignmentDao = new JdbcAssignmentDao(getConnectionManager());
            else
    ...
    public AuditLogDao getAuditLogDao() {
        if (this.cachedAuditLogDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedAuditLogDao = new JdbcAuditLogDao(getConnectionManager());
            else
                throw new RuntimeException("Invalid database type: " + getDatabaseType());
        }
    ...
    public HolidayDao getHolidayDao() {
        if (this.cachedHolidayDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedHolidayDao = new JdbcHolidayDao(getConnectionManager());
            else
                throw new RuntimeException("Invalid database type: " + getDatabaseType());
        }
    ...
    public PayPeriodDao getPayPeriodDao() {
        if (this.cachedPayPeriodDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedPayPeriodDao = new JdbcPayPeriodDao(getConnectionManager());
            else
                throw new RuntimeException("Invalid database type: " + getDatabaseType());
        }
    ...
    public TimesheetDao getTimesheetDao() {
        if (this.cachedTimesheetDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedTimesheetDao = new JdbcTimesheetDao(getConnectionManager());
            else
                throw new RuntimeException("Invalid database type: " + getDatabaseType());
        }
    ...
    /**
     * @return a {@link SupervisorDao} based on the currently configured database
     */
    public SupervisorDao getSupervisorDao() {
        if (this.cachedSupervisorDao == null) {
            if (DatabaseType.JDBC.equals(getDatabaseType()))
                this.cachedSupervisorDao = new JdbcSupervisorDao(getConnectionManager());
            else
                throw new RuntimeException("Invalid database type: " + getDatabaseType());
        }
