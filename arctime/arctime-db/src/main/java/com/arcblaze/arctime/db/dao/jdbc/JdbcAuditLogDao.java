...
public class JdbcAuditLogDao implements AuditLogDao {
     ...
     */
    @Override
    public Set<AuditLog> getForTimesheet(final Integer timesheetId) throws DatabaseException {
     ...
     */
    protected Map<Integer, Set<AuditLog>> getForTimesheets(final Connection conn, final Set<Integer> timesheetIds)
            ...
            for (final Integer timesheetId : timesheetIds) {
                ...
                try (final ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        Set<AuditLog> auditLogs = auditLogMap.get(auditLog.getTimesheetId());
                        if (auditLogs == null) {
     ...
     */
    @Override
    public int add(final Collection<AuditLog> auditLogs) throws DatabaseException {
        if (auditLogs == null || auditLogs.isEmpty())
            return 0;

        String sql = "INSERT INTO audit_logs (company_id, timesheet_id, log, " + "timestamp) VALUES (?, ?, ?, ?)";
        ...
        try (final Connection conn = this.connectionManager.getConnection();
            ...
            for (final AuditLog auditLog : auditLogs) {
                ...
                ps.setTimestamp(index++, new Timestamp(auditLog.getTimestamp().getTime()));
    ...
    public int delete(final Integer... timesheetIds) throws DatabaseException {
        return this.delete(timesheetIds == null ? null : Arrays.asList(timesheetIds));
    }
     ...
     */
    @Override
    public int delete(final Collection<Integer> timesheetIds) throws DatabaseException {
        if (timesheetIds == null || timesheetIds.isEmpty())
            return 0;
