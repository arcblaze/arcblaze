...
public class JdbcAssignmentDao implements AssignmentDao {
    ...
    protected static Assignment fromResultSet(final ResultSet rs) throws SQLException {
    ...
    public Assignment get(final Integer assignmentId) throws DatabaseException {
        ...
        final String sql = "SELECT a.*, u.id AS user_id, u.login, u.email, "
                ...
                + "t.job_code, t.admin FROM assignments a " + "JOIN users u ON (a.user_id = u.id) "
     ...
     */
    @Override
    public Set<Assignment> getForUser(final Integer companyId, final Integer userId, final Date day,
     ...
     */
    @Override
    public Set<Assignment> getForTask(final Integer companyId, final Integer taskId, final Date day,
            final boolean includeInactive) throws DatabaseException {
     ...
     */
    protected Set<Assignment> getForPayPeriod(final Connection conn, final Integer userId, final PayPeriod payPeriod)
    ...
    public int add(final Assignment... assignments) throws DatabaseException {
        return this.add(assignments == null ? null : Arrays.asList(assignments));
    }
     ...
     */
    @Override
    public int add(final Collection<Assignment> assignments) throws DatabaseException {
        ...
        try (final Connection conn = this.connectionManager.getConnection();
            ...
            for (final Assignment assignment : assignments) {
                ...
                if (assignment.getBegin() == null)
                    ...
                    ps.setTimestamp(index++, new Timestamp(assignment.getBegin().getTime()));
                if (assignment.getEnd() == null)
                    ps.setNull(index++, Types.DATE);
                else
                    ps.setTimestamp(index++, new Timestamp(assignment.getEnd().getTime()));
    ...
    public int update(final Assignment... assignments) throws DatabaseException {
        return this.update(assignments == null ? null : Arrays.asList(assignments));
    }
     ...
     */
    @Override
    public int update(final Collection<Assignment> assignments) throws DatabaseException {
        if (assignments == null || assignments.isEmpty())
            return 0;

        final String sql = "UPDATE assignments SET task_id = ?, user_id = ?, "
                + "labor_cat = ?, item_name = ?, begin = ?, end = ? " + "WHERE id = ?";
    ...
    public int delete(final Integer companyId, final Collection<Integer> ids) throws DatabaseException {
        ...
        final String sql = "DELETE FROM assignments " + "WHERE company_id = ? AND id = ?";
