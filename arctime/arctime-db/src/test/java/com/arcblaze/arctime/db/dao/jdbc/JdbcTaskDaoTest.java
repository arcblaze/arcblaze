...
public class JdbcTaskDaoTest {
	...
	public void dbIntegrationTests() throws DatabaseException {
		try (final TestDatabase database = new TestDatabase()) {
			...
			Set<Task> tasks = taskDao.getAll(company.getId(), true, true, null,
					null);
			assertNotNull(tasks);
			...
			getAllTasks = taskDao.getAll(company.getId(), false, true, null,
					null);
			assertNotNull(getAllTasks);
			...
			getAllTasks = taskDao.getAll(company.getId(), true, false, null,
					null);
			assertNotNull(getAllTasks);
			assertEquals(2, getAllTasks.size());
			assertTrue(getAllTasks.contains(task));
			...
			getAllTasks = taskDao.getAll(company.getId(), false, false, null,
					null);
			assertNotNull(getAllTasks);
			assertEquals(0, getAllTasks.size());

			getAllTasks = taskDao.getAll(company.getId(), true, true, null,
					null);
			assertNotNull(getAllTasks);
			assertEquals(2, getAllTasks.size());
			assertTrue(getAllTasks.contains(admin));
