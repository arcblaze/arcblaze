...
public class JdbcSupervisorDaoTest {
    ...
    public void dbIntegrationTests() throws DatabaseException {
        try (final TestDatabase database = new TestDatabase()) {
            ...
            final SupervisorDao supervisorDao = new JdbcSupervisorDao(database.getConnectionManager());
            ...
            Set<Supervisor> supervisors = supervisorDao.getSupervisors(company1.getId(), user.getId());
            assertNotNull(supervisors);
            assertTrue(supervisors.isEmpty());

            supervisorDao.add(company1.getId(), user.getId(), true, supervisor1.getId());
            supervisorDao.add(company1.getId(), user.getId(), false, supervisor2.getId());
            supervisorDao.add(company2.getId(), user.getId(), false, supervisor3.getId());

            supervisors = supervisorDao.getSupervisors(company1.getId(), user.getId());
            assertNotNull(supervisors);
            ...
            supervisorDao.delete(company1.getId(), user.getId(), supervisor2.getId());
            ...
            supervisorDao.delete(company1.getId(), user.getId(), supervisor1.getId());
