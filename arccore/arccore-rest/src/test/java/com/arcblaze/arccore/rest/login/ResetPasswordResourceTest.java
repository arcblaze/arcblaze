...
public class ResetPasswordResourceTest {
    ...
    public void testNullUser() {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            resource.reset(securityContext, config, daoFactory, password, timer, null);
        }
    }
    ...
    public void testBlankUser() {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final ResetPasswordResource resource = new ResetPasswordResource();
            resource.reset(securityContext, config, daoFactory, password, timer, "  ");
        }
    }
    ...
    public void testNonExistentUser() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final ResetPasswordResource resource = new ResetPasswordResource();
            resource.reset(securityContext, config, daoFactory, password, timer, "non-existent");
        }
    }

    /**
     * Test how the resource responds when an existing login is provided as input.
     * 
     * @throws DatabaseException
     *             if there is a database problem
     */
    ...
    public void testExistingUserByLogin() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final ResetPasswordMailSender mockMailSender = Mockito.mock(ResetPasswordMailSender.class);
            ...
            Mockito.when(mockPassword.hash("new-password", "new-salt")).thenReturn("hashed-password");

            final SecurityContext securityContext = Mockito.mock(SecurityContext.class);
            Mockito.when(securityContext.getUserPrincipal()).thenReturn(user);

            final ResetPasswordResource resource = new ResetPasswordResource(mockMailSender);
            resource.reset(securityContext, config, daoFactory, mockPassword, timer, user.getLogin());
    ...
    /**
     * Test how the resource responds when an existing email address is provided as input.
     * 
     * @throws DatabaseException
     *             if there is a database problem
     */
    ...
    public void testExistingUserByEmail() throws DatabaseException {
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            ResetPasswordResource resource = new ResetPasswordResource(mockMailSender);
            resource.reset(securityContext, config, daoFactory, mockPassword, timer, user.getEmail());
     ...
     */
    @Test
    public void testExistingUserEmailError() throws DatabaseException, MessagingException {
        final Config config = new Config();
        ...
        try (final TestDatabase testDatabase = new TestDatabase()) {
            ...
            final ResetPasswordMailSender mailSender = new ResetPasswordMailSender(config, user, "");
            final ResetPasswordMailSender mockMailSender = Mockito.spy(mailSender);
            Mockito.doThrow(MessagingException.class).when(mockMailSender).send();
