...
public class LoginResourceTest {
	/**
	 * Test how the resource responds when the provided login and password
	 * values are null.
	 * 
	 ...
	 */
	@Test
	public void testNullRedirectUri() throws URISyntaxException,
			ServletException {
		final HttpServletRequest request = Mockito
				.mock(HttpServletRequest.class);
		final UriInfo uriInfo = Mockito.mock(UriInfo.class);
		Mockito.when(uriInfo.getBaseUri()).thenReturn(
				new URI("http://localhost/rest/login"));
		final MetricRegistry metricRegistry = new MetricRegistry();
		final Timer timer = metricRegistry.timer("test");

		final LoginResource resource = new LoginResource();
		final Response response = resource.login(request, uriInfo, timer,
				"user", "password", null);
	 ...
	 */
	@Test
	public void testFullLoginNoException() throws URISyntaxException,
			ServletException {
		final HttpServletRequest request = Mockito
				.mock(HttpServletRequest.class);
		final UriInfo uriInfo = Mockito.mock(UriInfo.class);
	 ...
	 */
	@Test
	public void testFullLoginAlreadyLoggedIn() throws URISyntaxException,
			ServletException {
		final HttpServletRequest request = Mockito
				.mock(HttpServletRequest.class);
	 ...
	 */
	@Test
	public void testFullLoginSomeoneElseAlreadyLoggedIn()
	...
	public void testFullLoginWithException() throws URISyntaxException,
			ServletException {
		final HttpServletRequest request = Mockito
				.mock(HttpServletRequest.class);
		Mockito.doThrow(new ServletException("Login failed")).when(request)
				.login("user", "password");
		final UriInfo uriInfo = Mockito.mock(UriInfo.class);
		Mockito.when(uriInfo.getBaseUri()).thenReturn(
				new URI("http://localhost/rest/login"));
		...
		final LoginResource resource = new LoginResource();
		resource.login(request, uriInfo, timer, "user", "password",
				"/some/page.jsp");
	}
}
