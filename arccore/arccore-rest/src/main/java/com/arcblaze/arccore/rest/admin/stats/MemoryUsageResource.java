...
public class MemoryUsageResource extends BaseResource {
	...
	public Memory nonheap(@Context final Timer timer) {
		try (final Timer.Context timerContext = timer.time()) {
			final MemoryUsage nonHeap = this.memoryMXBean
	...
	public MemoryInfo info(@Context final Timer timer) {
		try (final Timer.Context timerContext = timer.time()) {
			...
			info.maxHeap = new BigDecimal(maxHeap).setScale(2,
					BigDecimal.ROUND_HALF_UP);
			info.heapPctUsed = new BigDecimal(heapPctUsed).setScale(2,
					BigDecimal.ROUND_HALF_UP);
			...
			info.maxNonHeap = new BigDecimal(maxNonHeap).setScale(2,
					BigDecimal.ROUND_HALF_UP);
			info.nonHeapPctUsed = new BigDecimal(nonHeapPctUsed).setScale(2,
					BigDecimal.ROUND_HALF_UP);
