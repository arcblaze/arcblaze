...
public class ActiveCompaniesResource extends BaseResource {
    ...
    public String activeCompanies(@Context final DaoFactory daoFactory, @Context final Timer timer,
            @QueryParam("begin") final String beginStr, @QueryParam("end") final String endStr)
            throws DatabaseException {
        try (final Timer.Context timerContext = timer.time()) {
            final Date begin = StringUtils.isBlank(beginStr) ? DateUtils.truncate(DateUtils.addDays(new Date(), -365),
                    Calendar.MONTH) : DateUtils.parseDate(beginStr, FMT);
            final Date end = StringUtils.isBlank(endStr) ? DateUtils.addDays(new Date(), 1) : DateUtils.parseDate(
                    endStr, FMT);

            final CompanyActivityDao dao = daoFactory.getCompanyActivityDao();
            final SortedMap<Date, Integer> data = dao.getActiveByMonth(begin, end);
