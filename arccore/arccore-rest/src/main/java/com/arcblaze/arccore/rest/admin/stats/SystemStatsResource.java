	...
	public Stats getStats(@Context final DaoFactory daoFactory,
		...
		try (final Timer.Context timerContext = timer.time()) {
			final List<FutureTask<SystemStat>> tasks = Arrays.asList(
					getRevenueYTD(daoFactory), getRevenueYear(daoFactory),
					getActiveUsers(daoFactory), getActiveCompanies(daoFactory));
	...
	protected FutureTask<SystemStat> getRevenueYTD(final DaoFactory daoFactory) {
		return new FutureTask<>(new Callable<SystemStat>() {
			...
			public SystemStat call() throws DatabaseException {
				final TransactionDao transactionDao = daoFactory
						.getTransactionDao();
