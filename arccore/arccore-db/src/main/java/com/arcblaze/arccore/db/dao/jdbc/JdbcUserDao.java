...
public class JdbcUserDao implements UserDao {
    ...
    protected User fromResultSet(final ResultSet rs, final boolean includePass) throws SQLException {
    ...
    public int count(final boolean includeInactive) throws DatabaseException {
        final String sql = "SELECT COUNT(*) FROM users" + (includeInactive ? "" : " WHERE active = TRUE");
     ...
     */
    @Override
    public int count(final String filter, final boolean includeInactive) throws DatabaseException {
     ...
     */
    @Override
    public Map<Integer, Integer> countPerCompany(final boolean includeInactive) throws DatabaseException {
        final String sql = "SELECT company_id, COUNT(*) AS count FROM users"
                + (includeInactive ? "" : " WHERE active = TRUE") + " GROUP BY company_id";
    ...
    public User getLogin(final String login) throws DatabaseException {
        ...
        final String sql = "SELECT * FROM users WHERE active = true AND " + "(login = ? OR LOWER(email) = LOWER(?))";
     ...
     */
    @Override
    public User get(final Integer companyId, final Integer userId) throws DatabaseException {
        notNull(companyId, "Invalid null company id");
        notNull(userId, "Invalid null user id");

        final String sql = "SELECT * FROM users " + "WHERE company_id = ? AND id = ?";
     ...
     */
    @Override
    public Map<Integer, User> get(final Set<Integer> ids) throws DatabaseException {
     ...
     */
    @Override
    public Set<User> getAll(final Integer companyId, final boolean includeInactive, final Integer limit,
     ...
     */
    @Override
    public Set<User> getAll(final boolean includeInactive, final Integer limit, final Integer offset)
     ...
     */
    @Override
    public Set<User> search(final Integer companyId, final String filter, final boolean includeInactive,
     ...
     */
    @Override
    public Set<User> search(final String filter, final boolean includeInactive, final Integer limit,
            final Integer offset) throws DatabaseException {
        final StringBuilder sql = new StringBuilder();
        sql.append("SELECT * FROM users");
        if (StringUtils.isNotBlank(filter)) {
     ...
     */
    @Override
    public int add(final User... users) throws DatabaseUniqueConstraintException, DatabaseException {
     ...
     */
    @Override
    public int add(final Collection<User> users) throws DatabaseUniqueConstraintException, DatabaseException {
    ...
    public int activate(final Integer companyId, final Collection<Integer> ids) throws DatabaseException {
        ...
        final String sql = "UPDATE users SET active = true " + "WHERE company_id = ? AND id = ?";
    ...
    public int deactivate(final Integer companyId, final Collection<Integer> ids) throws DatabaseException {
        ...
        final String sql = "UPDATE users SET active = false " + "WHERE company_id = ? AND id = ?";
     ...
     */
    @Override
    public int update(final User... users) throws DatabaseUniqueConstraintException, DatabaseException {
     ...
     */
    @Override
    public int update(final Collection<User> users) throws DatabaseUniqueConstraintException, DatabaseException {
        if (users == null || users.isEmpty())
            return 0;

        // NOTE: the hashed_pass and salt values are not updated.

        final String sql = "UPDATE users SET login = ?, email = ?, " + "first_name = ?, last_name = ?, active = ? "
     ...
     */
    @Override
    public int setPassword(final Integer userId, final String hashedPass, final String salt) throws DatabaseException {
        notNull(userId, "Invalid null user id");
        notEmpty(hashedPass, "Invalid blank password");
        notEmpty(salt, "Invalid blank salt");

        final String sql = "UPDATE users SET hashed_pass = ?, salt = ? " + "WHERE id = ?";
