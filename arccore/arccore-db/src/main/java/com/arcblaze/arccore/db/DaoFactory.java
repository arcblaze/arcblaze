...
public class DaoFactory {
	...
	public CompanyDao getCompanyDao() {
		if (this.cachedCompanyDao == null) {
			if (DatabaseType.JDBC.equals(this.databaseType))
				this.cachedCompanyDao = new JdbcCompanyDao(
	...
	/**
	 * @return a {@link CompanyActivityDao} based on the currently configured
	 *         database
	 */
	public CompanyActivityDao getCompanyActivityDao() {
		if (this.cachedCompanyActivityDao == null) {
			if (DatabaseType.JDBC.equals(getDatabaseType()))
				this.cachedCompanyActivityDao = new JdbcCompanyActivityDao(
						getConnectionManager());
			else
				throw new RuntimeException("Invalid database type: "
						+ getDatabaseType());
	...
	/**
	 * @return a {@link UserActivityDao} based on the currently configured
	 *         database
	 */
	public UserActivityDao getUserActivityDao() {
		if (this.cachedUserActivityDao == null) {
			if (DatabaseType.JDBC.equals(getDatabaseType()))
				this.cachedUserActivityDao = new JdbcUserActivityDao(
						getConnectionManager());
			else
				throw new RuntimeException("Invalid database type: "
						+ getDatabaseType());
