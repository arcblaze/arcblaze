...
/**
 * Responsible for periodically performing health checks on all system components.
 */
public class SystemHealthCheckTask extends BackgroundTask {
    private final static Logger log = LoggerFactory.getLogger(SystemHealthCheckTask.class);
     ...
     */
    public SystemHealthCheckTask(final Config config, final MetricRegistry metricRegistry,
    ...
    public void process() throws BackgroundTaskException {
        final SortedMap<String, Result> results = getHealthCheckRegistry().runHealthChecks();
        ...
        if (unhealthy > 0)
            log.warn("Health Checks: {} healthy, {} unhealthy", healthy, unhealthy);
        else
            log.info("Health Checks: {} healthy, {} unhealthy", healthy, unhealthy);
