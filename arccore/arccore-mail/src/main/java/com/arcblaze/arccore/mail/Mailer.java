...
/**
 * Responsible for sending emails. Package private since it is only meant to be
 * used inside {@link MailSender} objects.
 */
class Mailer {
	 ...
	 */
	void send(final Session session, final MimeMessage message)
			throws MessagingException {
		if (this.config.getBoolean(MailProperty.SMTP_MAIL_AUTHENTICATE)) {
